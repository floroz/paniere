          ...
          {isLoading ? (
            ...
            <div className="py-2">
              <ReactMarkdown components={{
                h1: ({ children, ...props }) => (
                  <h1 {...props} className="text-3xl font-serif text-amber-800 dark:text-amber-300 font-bold mt-6 mb-4">
                    {children}
                  </h1>
                ),
                h2: ({ children, ...props }) => (
                  <h2 {...props} className="text-2xl font-serif text-amber-700 dark:text-amber-400 font-bold mt-8 mb-4 pb-2 border-b border-amber-200 dark:border-amber-800/30">
                    {children}
                  </h2>
                ),
                h3: ({ children, ...props }) => (
                  <h3 {...props} className="text-xl font-serif text-amber-700 dark:text-amber-400 font-semibold mt-6 mb-3">
                    {children}
                  </h3>
                ),
                p: ({ children, ...props }) => (
                  <p {...props} className="text-gray-700 dark:text-gray-300 my-4">
                    {children}
                  </p>
                ),
                ul: ({ children, ...props }) => (
                ...
                li: ({ children, ...props }) => (
                  <li {...props} className="my-1 text-gray-700 dark:text-gray-300">
                    {children}
                  </li>
                ),
                strong: ({ children, ...props }) => (
                  <strong {...props} className="font-bold text-amber-700 dark:text-amber-300">
                    {children}
                  </strong>
                ),
                em: ({ children, ...props }) => (
                ...
                a: ({ children, ...props }) => (
                  <a {...props} className="text-amber-600 dark:text-amber-400 hover:underline">
                    {children}
                  </a>
                ),
                blockquote: ({ children, ...props }) => (
                  <blockquote {...props} className="border-l-4 border-amber-300 dark:border-amber-700 pl-4 py-1 my-4 italic text-gray-600 dark:text-gray-400">
                    {children}
                  </blockquote>
                ),
                code: ({ children, ...props }) => (
                  <code {...props} className="bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 rounded text-amber-700 dark:text-amber-300 font-mono text-sm">
                    {children}
                  </code>
                ),
                pre: ({ children, ...props }) => (
                  <pre {...props} className="bg-gray-100 dark:bg-gray-800 p-4 rounded-md overflow-auto my-4">
                    {children}
                  </pre>
                ),
                hr: ({ ...props }) => (
                  <hr {...props} className="my-8 border-t border-amber-200 dark:border-amber-800/30" />
                ),
              }}>
                {rules}
              </ReactMarkdown>
