    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4 md:p-8 bg-gradient-to-br from-amber-50 via-white to-amber-100 dark:from-gray-950 dark:via-gray-900 dark:to-amber-950 text-gray-900 dark:text-gray-100">
      ...
      <main className="relative z-10 max-w-4xl mx-auto w-full flex flex-col items-center justify-center gap-8">
        ...
        <section className="w-full grid grid-cols-1 md:grid-cols-2 gap-6">
          <h2 className="col-span-full text-2xl font-bold mb-4 bg-gradient-to-r from-amber-600 to-amber-800 dark:from-amber-500 dark:to-amber-700 bg-clip-text text-transparent font-display">
            {t.selectMode}
          </h2>
          ...
          <button
            ...
            tabIndex={0}
            onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && setSelectedMode('tabellone')}
          >
            ...
            <h3 className="text-xl font-bold mb-2 text-amber-700 dark:text-amber-500 font-display" id="tabellone-mode-label">
              {t.tabelloneMode}
            </h3>
            <p className="text-sm text-gray-600 dark:text-gray-300 text-center" aria-labelledby="tabellone-mode-label">
          ...
          <button
            ...
            tabIndex={0}
            onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && setSelectedMode('player')}
          >
            <div className="w-24 h-24 mb-4 flex items-center justify-center rounded-full bg-amber-500/10 dark:bg-amber-700/20">
              <CartellaIcon 
                className="text-amber-600 dark:text-amber-500 transform scale-125" 
            ...
            <h3 className="text-xl font-bold mb-2 text-amber-700 dark:text-amber-500 font-display" id="player-mode-label">
              {t.playerMode}
            </h3>
            <p className="text-sm text-gray-600 dark:text-gray-300 text-center" aria-labelledby="player-mode-label">
              {t.playerDescription}
            </p>
          </button>
        </section>
              ...
              <div className="flex gap-3 flex-wrap justify-center">
                {Array.from({ length: 10 }, (_, idx) => idx + 1).map(count => (
                  <button
                    ...
                    tabIndex={0}
                    onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && setCartelleCount(count)}
                  >
                    {count}
                  </button>
                ))}
        ...
        <div className="w-full flex flex-col items-center mt-6">
          <button
            ...
            tabIndex={0}
            onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && selectedMode && handleStartGame()}
          >
            {selectedMode && (
              <>
